...
void LinuxNetworkSource::sample(std::vector<Reading>& out)
{
    ...
    for (const auto& kv : ordinals_)
    {
        ...
        // Live link state (like Task Manager's show/hide): carrier up.
        std::string oper = lnx::readTrim("/sys/class/net/" + n + "/operstate");
        out.push_back(Reading{dev, Quantity::Count, Unit::None, "Link", oper == "up" ? 1.0 : 0.0});
        uint64_t rx = 0, tx = 0;

...
void MacMemorySource::sample(std::vector<Reading>& out) {
    ...
    uint64_t avail = uint64_t(vm.free_count + vm.inactive_count + vm.speculative_count +
                              vm.purgeable_count) *
                     page;
    if (avail > total)
        avail = total;
    ...
    emit(Quantity::Load, Unit::Percent, "Usage", total ? 100.0 * double(used) / double(total) : 0.0);

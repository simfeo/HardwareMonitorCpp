...
struct TelemetryItem {
    bool bSupported;
    int32_t units;
    int32_t type; // ctl_data_type_t: 0=int8,1=uint8,2=int16,3=uint16,4=int32,5=uint32,6=int64,7=uint64,8=float,9=double
    CtlValue value;
};
...
Igcl::Igcl() {
    ...
    t->enumerate =
        reinterpret_cast<FnEnumerate>(reinterpret_cast<void*>(GetProcAddress(m, "ctlEnumerateDevices")));
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        AdapterRef a;
        a.handle = handles[i];
        a.name = "Intel GPU"; // ctl_device_adapter_properties_t name read is added during validation
        a.discrete = true;

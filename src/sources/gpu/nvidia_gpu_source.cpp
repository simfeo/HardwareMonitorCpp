...
void NvidiaGpuSource::sample(std::vector<Reading>& out)
{
    ...
    for (size_t i = 0; i < handles_.size(); ++i)
    {
        ...
        if (nvml_->memory(dev, m))
        {
            ...
            if (m.total)
            {
                emit(Quantity::Load, Unit::Percent, "Memory",
                     100.0 * double(m.used) / double(m.total));
            }
        }
        unsigned mhz = 0;
        if (nvml_->clockMhz(dev, 0, mhz))
        {
            emit(Quantity::Clock, Unit::Megahertz, "Core", mhz);
        }
        if (nvml_->clockMhz(dev, 2, mhz))
        {
            emit(Quantity::Clock, Unit::Megahertz, "Memory", mhz);
        }
        unsigned fan = 0;

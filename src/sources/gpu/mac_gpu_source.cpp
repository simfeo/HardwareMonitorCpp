...
void MacGpuSource::sample(std::vector<Reading>& out)
{
    ...
    CFTypeRef stats = IORegistryEntryCreateCFProperty(service_, CFSTR("PerformanceStatistics"),
                                                      kCFAllocatorDefault, 0);
    if (stats && CFGetTypeID(stats) == CFDictionaryGetTypeID())
    {
        auto d = (CFDictionaryRef)stats;
        double v = 0;
        if (mac::dictNumber(d, "Device Utilization %", v))
        {
            emit(Quantity::Load, Unit::Percent, "Core", v);
        }
        if (mac::dictNumber(d, "Renderer Utilization %", v))
        {
            emit(Quantity::Load, Unit::Percent, "Renderer", v);
        }
        if (mac::dictNumber(d, "Tiler Utilization %", v))
        {
            emit(Quantity::Load, Unit::Percent, "Tiler", v);
        }
        if (mac::dictNumber(d, "In use system memory", v))
        {
            emit(Quantity::DataVolume, Unit::Byte, "Memory In Use", v);
        }
    }
    if (stats)
    {
    ...
    // Temperature: count only plausible readings (the Tg sensors floor sub-ambient when
    // powergated).
    if (!tempKeys_.empty())
    {
        double sum = 0;
        ...
        {
            float t;
            if (smc_.readFloat(k, t) && t > 20.0f && t < 130.0f)
            {
                sum += t;
                ++n;
            }
    ...
    if (ioreport_.ok())
    {
        ...
        {
            if (s.powerW.count("GPU"))
            {
                emit(Quantity::Power, Unit::Watt, "Power", s.powerW["GPU"]);
            }
            if (s.freqMhz.count("GPU"))
            {
                emit(Quantity::Clock, Unit::Megahertz, "Core", s.freqMhz["GPU"]);
            }
        }
    }
}

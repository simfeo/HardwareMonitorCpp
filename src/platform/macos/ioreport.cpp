...
IoReport::IoReport()
{
    ...
    if (merge)
    {
        if (auto c = copyChannels(CFSTR("CPU Stats"), nullptr, 0, 0, 0))
        {
            merge(chans, c, nullptr);
            CFRelease(c);
        }
        if (auto g = copyChannels(CFSTR("GPU Stats"), nullptr, 0, 0, 0))
        {
            merge(chans, g, nullptr);
            CFRelease(g);
        }
    }
    ...
    if (d)
    {
        iterate(d, ^int(CFDictionaryRef ch) {
          ...
          if (group == "Energy Model")
          {
              ...
              else if (name.find("ANE") != std::string::npos)
              {
                  joule["ANE"] += j;
              }
